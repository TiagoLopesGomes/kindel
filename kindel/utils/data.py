...
from kindel.utils.fingerprint_feat import CircularFingerprint, CombinedFingerprint, ChemBERTaFeaturizer
...
def featurize(df, smiles_col, label_col=None, featurizer_type="combined"):
    if featurizer_type == "morgan":
        fingerprinter = CircularFingerprint()
    elif featurizer_type == "combined":
        fingerprinter = CombinedFingerprint()
    elif featurizer_type == "chemberta":
        fingerprinter = ChemBERTaFeaturizer()
    else:
...
def get_training_data(target, split_index, split_type):
    ...
    print("########################################################")
    print(f"Loading splits from {DATA_ROOT}/splits")
    print("########################################################")
